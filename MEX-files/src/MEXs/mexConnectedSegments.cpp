...
{   
    ...
    superPixels_OUT = mxCreateNumericArray(2, dims, mxINT32_CLASS, mxREAL);
    int* superPixels = (int*)mxGetData(superPixels_OUT);

    for(unsigned int i = 0; i < imgRows*imgCols; ++i)
        superPixels[i] = segmentation[i];

    /* Call method: ConnectedSegments */
    superPixelSegmentation(imgRows, imgCols, imgChannels, superPixels, image, Z, u, v, w);
